...
# Настройки Bright Data берем из переменных окружения (но оставляем твои значения по умолчанию)
UNLOCKER_USER = os.getenv("UNLOCKER_USER", "brd-customer-hl_af9d374d-zone-web_unlocker")
UNLOCKER_PASS = os.getenv("UNLOCKER_PASS", "41lvtwmav2y8")
UNLOCKER_HOST = os.getenv("UNLOCKER_HOST", "brd.superproxy.io")
UNLOCKER_PORT = os.getenv("UNLOCKER_PORT", "33335")
...
def scrape_schedule():
    ...
    try:
        ...
        response = requests.get(
            url, proxies=WEB_UNLOCKER_PROXY, timeout=120, verify=False
        )
        ...
        if response.status_code == 200:
            ...
            if container:
                    ...
                    if text and len(text) > 3:
                        if "З" in text and "до" in text:
                            ...
                            time_match = re.search(
                                r"З\s*(\d{2}):(\d{2})\s*до\s*(\d{2}):(\d{2})", text
                            )
                            if time_match:
                ...
                if not result_lines:
                    result_lines = [
                        "✅ На сегодня отключений не найдено (или сайт изменился)."
                    ]
